...
const SceneAppBook = ({navigation}) => {
    const dispatch=useDispatch()
    const {categoryproducts,loading}=useSelector((state)=>state.product)
    const route=useRoute()
    useEffect(()=>{
        const categoryName=route.params?.categoryName||'Book'
    ...
    <DefaultPage>
        ...
        <Text style={styles.headline}>Book Product</Text>
            {loading?(
                <><Text>Loading ...</Text></>
            ):(
                <>
...
SceneAppBook.propTypes = {
    navigation: PropTypes.object.isRequired,
}

export default SceneAppBook